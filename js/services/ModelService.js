angular.module('jsmusicdb.modelService', []).service('modelService', function($rootScope, $http, $timeout, $log) {'use strict';
	var that = this;
		...
		switch (value.Type) {
			...
			case 'artist':
				if (value.Naam) {
					...
					$log.warn('No \'Naam\' field found, we had ' + JSON.stringify(value));
				}
				break;
			case 'album':
			...
			default:
				...
				if (window.console)
					$log.warn('Unknown type: ' + value.Type);
